...
namespace ShaderMapDiagnostics
{
    namespace
    {
        ...
        const TCHAR *const ReplacementDefaultPostProcessPath =
            TEXT("/TajsGraph/Engine/EngineMaterials/DefaultPostProcessMaterial");

        using FCacheShadersSignature = bool (FMaterial::*)(EShaderPlatform, EMaterialShaderPrecompileMode,
                                                           const ITargetPlatform *);
        using FCacheShadersWithIdSignature = bool (FMaterial::*)(const FMaterialShaderMapId &, EShaderPlatform,
                                                                 EMaterialShaderPrecompileMode,
                                                                 const ITargetPlatform *);
        using FTryGetShadersSignature = bool (FMaterial::*)(const FMaterialShaderTypes &, const FVertexFactoryType *,
                                                            FMaterialShaders &) const;
        using FGetShaderPipelineSignature = FShaderPipelineRef (FMaterial::*)(FShaderPipelineType *,
                                                                              FVertexFactoryType *, bool) const;
        ...
        private:
            ...
            bool LoadConfig()
            {
                ...
                for (const FString &ConfigPath : ConfigCandidates)
                {
                    ...
                    bool bValue = false;
                    if (GConfig->GetBool(TajsGraphEarlyShared::GetConfigSection(),
                                         TajsGraphEarlyShared::GetEnableMaterialShaderMapDiagKey(), bValue, ConfigPath))
                    {
                        bEnabled = bValue;
                    }
                    ...
                    if (GConfig->GetBool(TajsGraphEarlyShared::GetConfigSection(),
                                         TajsGraphEarlyShared::GetEnableMaterialShaderMapHashRemapKey(),
                                         bHashRemapValue, ConfigPath))
                    {
            ...
            void LoadAssetInfoIndex()
            {
                ...
                {
                    const FString AssetInfoFilename = FString::Printf(TEXT("ShaderAssetInfo-%s-%s-%s.assetinfo.json"),
                                                                      *LibraryName, *PlatformName, *PlatformName);
                ...
                if (!bLoadedAnyAssetInfo)
                {
                    UE_LOG(LogTajsGraphEarlyDiag, Warning,
                           TEXT("[ShaderDiag][Startup] No assetinfo index files were found for FactoryExtra or "
                                "TajsGraph; expected hash comparison will be unavailable."));
                }
            }
            ...
            void LogStartupSummary() const
            {
                ...
                const FString *ReplacementPostProcessHash =
                    ExpectedHashByAssetPath.Find(ReplacementDefaultPostProcessPath);

                UE_LOG(LogTajsGraphEarlyDiag, Display,
                       TEXT("[ShaderDiag][Startup] Enabled=true IndexedAssets=%d IndexedHashes=%d HashRemapMode=%s "
                            "WorldGridHash=%s EngineDefaultPostProcessHash=%s TajsGraphDefaultPostProcessHash=%s"),
                       ExpectedHashByAssetPath.Num(), AssetPathsByHash.Num(),
            ...
            void InstallHooks()
            {
                ...
                CacheShadersHandle = SUBSCRIBE_METHOD_EXPLICIT(
                    FCacheShadersSignature, FMaterial::CacheShaders,
                    [this](auto &Scope, FMaterial *Material, EShaderPlatform Platform,
                           EMaterialShaderPrecompileMode PrecompileMode, const ITargetPlatform *TargetPlatform)
                    {
                        Scope(Material, Platform, PrecompileMode, TargetPlatform);
                        OnCacheShaders(Material, Platform);
                    });

                CacheShadersWithIdHandle = SUBSCRIBE_METHOD_EXPLICIT(
                    FCacheShadersWithIdSignature, FMaterial::CacheShaders,
                    [this](auto &Scope, FMaterial *Material, const FMaterialShaderMapId &ShaderMapId,
                ...
                TryGetShadersHandle = SUBSCRIBE_METHOD_EXPLICIT(
                    FTryGetShadersSignature, FMaterial::TryGetShaders,
                    [this](auto &Scope, const FMaterial *Material, const FMaterialShaderTypes &ShaderTypes,
                           const FVertexFactoryType *VertexFactoryType, FMaterialShaders &OutShaders)
                    {
                ...
                GetShaderPipelineHandle = SUBSCRIBE_METHOD_EXPLICIT(
                    FGetShaderPipelineSignature, FMaterial::GetShaderPipeline,
                    [this](auto &Scope, const FMaterial *Material, FShaderPipelineType *ShaderPipelineType,
                           FVertexFactoryType *VertexFactoryType, bool bFatalIfNotFound)
                    {
                        OnShaderPipelineLookup(Scope, Material, ShaderPipelineType, VertexFactoryType,
                                               bFatalIfNotFound);
                ...
                UE_LOG(LogTajsGraphEarlyDiag, Display,
                       TEXT("[ShaderDiag][Startup] Installed native material/shader-map hooks."));
            }
            ...
            void UninstallHooks()
            {
                ...
                {
                    UNSUBSCRIBE_METHOD_EXPLICIT(FCacheShadersWithIdSignature, FMaterial::CacheShaders,
                                                CacheShadersWithIdHandle);
                ...
                {
                    UNSUBSCRIBE_METHOD_EXPLICIT(FGetShaderPipelineSignature, FMaterial::GetShaderPipeline,
                                                GetShaderPipelineHandle);
            ...
            void OnCacheShaders(FMaterial *Material, EShaderPlatform Platform)
            {
                ...
                const FString Key = FString::Printf(TEXT("RuntimeHash|%s|%s|%s"), *Context.AssetPath, *Context.Platform,
                                                    *Context.RuntimeHash);
                if (!MarkEmitted(Key))
                {
                    return;
                ...
                UE_LOG(LogTajsGraphEarlyDiag, Display,
                       TEXT("[ShaderDiag][RuntimeHash] Material=%s AssetPath=%s Platform=%s RuntimeHash=%s "
                            "ExpectedIndexedHash=%s Match=%s"),
                       Context.MaterialName.IsEmpty() ? TEXT("<unknown>") : *Context.MaterialName,
                       Context.AssetPath.IsEmpty() ? TEXT("<unknown>") : *Context.AssetPath,
                       Context.Platform.IsEmpty() ? TEXT("<unknown>") : *Context.Platform, *Context.RuntimeHash,
                       Context.ExpectedHash.IsEmpty() ? TEXT("<missing>") : *Context.ExpectedHash, *Context.MatchState);
            }

            void OnTryGetShaders(TCallScope<bool (*)(const FMaterial *, const FMaterialShaderTypes &,
                                                     const FVertexFactoryType *, FMaterialShaders &)> &Scope,
                                 const FMaterial *Material, const FMaterialShaderTypes &ShaderTypes,
                                 const FVertexFactoryType *VertexFactoryType, FMaterialShaders &OutShaders)
            {
                ...
                const FRuntimeMaterialContext Context =
                    BuildMaterialContext(Material, ShaderMap, ShaderMap->GetShaderPlatform());
                EmitLookupMiss(Context, JoinRequestedShaderTypeNames(ShaderTypes),
                ...
                if (TryRecoverShadersFromExpectedHash(Context, Material, ShaderMap, ShaderTypes, VertexFactoryType,
                                                      OutShaders))
                {
                    Scope.Override(true);
                }
            ...
            void OnShaderPipelineLookup(TCallScope<FShaderPipelineRef (*)(const FMaterial *, FShaderPipelineType *,
                                                                          FVertexFactoryType *, bool)> &Scope,
                                        const FMaterial *Material, FShaderPipelineType *ShaderPipelineType,
                                        FVertexFactoryType *VertexFactoryType, bool bFatalIfNotFound)
            {
                ...
                const bool bFoundPipeline =
                    MeshShaderMap && MeshShaderMap->GetShaderPipeline(ShaderPipelineType) != nullptr;
                if (bFoundPipeline)
                {
                    return;
                }
                ...
                const FRuntimeMaterialContext Context =
                    BuildMaterialContext(Material, ShaderMap, ShaderMap->GetShaderPlatform());
                EmitLookupMiss(Context, TEXT("<none>"), ShaderPipelineType->GetName(), VertexFactoryType->GetName(), 0,
                               TEXT("Pipeline"), bFatalIfNotFound ? TEXT("FatalPath") : TEXT("NonFatalPath"));

                FShaderPipelineRef RemappedPipeline;
                if (TryRecoverPipelineFromExpectedHash(Context, Material, ShaderMap, ShaderPipelineType,
                                                       VertexFactoryType, RemappedPipeline))
                {
                    Scope.Override(RemappedPipeline);
                }
            }

            FRuntimeMaterialContext BuildMaterialContext(const FMaterial *Material, const FMaterialShaderMap *ShaderMap,
                                                         EShaderPlatform Platform) const
            {
            ...
            FRuntimeMaterialContext ResolveRuntimeContextByHash(const FString &RuntimeHash,
                                                                EShaderPlatform Platform) const
            {
                FScopeLock Lock(&StateMutex);
            ...
            {
                return Context.AssetPath == DefaultWorldGridPath || Context.AssetPath == DefaultPostProcessPath ||
            ...
            {
                return bEnableHashRemap && Context.MatchState == TEXT("mismatch") && !Context.ExpectedHash.IsEmpty() &&
            ...
            bool TryRecoverShadersFromExpectedHash(const FRuntimeMaterialContext &Context, const FMaterial *Material,
                                                   ...
                                                   FMaterialShaders &OutShaders)
            {
                ...
                EmitHashRemapUnsupported(Context, JoinRequestedShaderTypeNames(ShaderTypes),
                                         ShaderTypes.PipelineType ? ShaderTypes.PipelineType->GetName()
                                                                  : TEXT("<none>"),
                                         VertexFactoryType ? VertexFactoryType->GetName() : TEXT("<none>"),
                                         ExtractPrimaryPermutationId(ShaderTypes), TEXT("TryGetShaders"));
                return false;
            }

            bool TryRecoverPipelineFromExpectedHash(const FRuntimeMaterialContext &Context, const FMaterial *Material,
                                                    const FMaterialShaderMap *ShaderMap,
                                                    FShaderPipelineType *ShaderPipelineType,
                                                    FVertexFactoryType *VertexFactoryType,
                                                    FShaderPipelineRef &OutPipeline)
            {
                if (!ShouldAttemptHashRemap(Context) || !Material || !ShaderMap || !ShaderPipelineType ||
                    !VertexFactoryType)
                {
                    return false;
                }

                EmitHashRemapUnsupported(Context, TEXT("<none>"), ShaderPipelineType->GetName(),
                                         VertexFactoryType->GetName(), 0, TEXT("GetShaderPipeline"));
                return false;
            }

            void EmitHashRemapUnsupported(const FRuntimeMaterialContext &Context, const FString &ShaderType,
                                          const FString &PipelineType, const FString &VertexFactory,
                                          int32 PermutationId, const FString &DetailSource)
            {
                const FString Key = FString::Printf(TEXT("HashRemapUnsupported|%s|%s|%s|%s|%s|%d"), *Context.AssetPath,
                                                    *Context.RuntimeHash, *Context.ExpectedHash, *ShaderType,
                                                    *PipelineType, PermutationId);
                if (!MarkEmitted(Key))
                {
                    return;
                }
                ...
                UE_LOG(LogTajsGraphEarlyDiag, Warning,
                       TEXT("[ShaderDiag][HashRemapUnsupported] Material=%s AssetPath=%s RuntimeHash=%s "
                            "ExpectedIndexedHash=%s ShaderType=%s PipelineType=%s VertexFactory=%s Platform=%s "
                            "PermutationId=%d DetailSource=%s"),
                       Context.MaterialName.IsEmpty() ? TEXT("<unknown>") : *Context.MaterialName,
                       Context.AssetPath.IsEmpty() ? TEXT("<unknown>") : *Context.AssetPath,
                       ...
                       Context.Platform.IsEmpty() ? TEXT("<unknown>") : *Context.Platform, PermutationId,
                       *DetailSource);
            }
            ...
            void EmitLookupMiss(const FRuntimeMaterialContext &Context, const FString &ShaderType,
                                const FString &PipelineType, const FString &VertexFactory, int32 PermutationId,
                                const FString &KeyKind, const FString &DetailSource)
            {
                const FString Key =
                    FString::Printf(TEXT("LookupMiss|%s|%s|%s|%s|%s|%d"), *Context.AssetPath, *Context.RuntimeHash,
                                    *ShaderType, *PipelineType, *VertexFactory, PermutationId);
                if (!MarkEmitted(Key))
                {
                    return;
                }
                ...
                UE_LOG(LogTajsGraphEarlyDiag, Warning,
                       TEXT("[ShaderDiag][LookupMiss] Material=%s AssetPath=%s RuntimeHash=%s ExpectedIndexedHash=%s "
                            "Match=%s ShaderType=%s PipelineType=%s VertexFactory=%s Platform=%s PermutationId=%d "
                            "KeyKind=%s DetailSource=%s"),
                       Context.MaterialName.IsEmpty() ? TEXT("<unknown>") : *Context.MaterialName,
                       Context.AssetPath.IsEmpty() ? TEXT("<unknown>") : *Context.AssetPath,
                       Context.RuntimeHash.IsEmpty() ? TEXT("<unavailable>") : *Context.RuntimeHash,
                       Context.ExpectedHash.IsEmpty() ? TEXT("<missing>") : *Context.ExpectedHash,
                       ...
                       PipelineType.IsEmpty() ? TEXT("<none>") : *PipelineType,
                       VertexFactory.IsEmpty() ? TEXT("<none>") : *VertexFactory,
                       Context.Platform.IsEmpty() ? TEXT("<unknown>") : *Context.Platform, PermutationId, *KeyKind,
                       *DetailSource);
            }
            ...
            bool LoadAssetInfoFile(const FString &AssetInfoPath, const FString &SourceLabel, const FString &SourceKind)
            {
                ...
                {
                    UE_LOG(LogTajsGraphEarlyDiag, Warning,
                           TEXT("[ShaderDiag][Startup] Failed to read %s assetinfo file %s"), *SourceLabel,
                           *AssetInfoPath);
                    return false;
                }
                ...
                {
                    UE_LOG(LogTajsGraphEarlyDiag, Warning,
                           TEXT("[ShaderDiag][Startup] Failed to parse %s assetinfo JSON %s"), *SourceLabel,
                           *AssetInfoPath);
                    return false;
                }
                ...
                {
                    UE_LOG(LogTajsGraphEarlyDiag, Warning,
                           TEXT("[ShaderDiag][Startup] Missing ShaderCodeToAssets array in %s assetinfo JSON %s"),
                           *SourceLabel, *AssetInfoPath);
                    return false;
                }
                ...
                for (const TSharedPtr<FJsonValue> &EntryValue : *Entries)
                {
                    ...
                    if (!EntryObject->TryGetStringField(TEXT("ShaderMapHash"), ShaderMapHash) ||
                        ShaderMapHash.IsEmpty())
                    {
                        continue;
                    }
                ...
                UE_LOG(LogTajsGraphEarlyDiag, Display,
                       TEXT("[ShaderDiag][Startup] Loaded %s assetinfo index from %s [source=%s] (%d entries, %d "
                            "hashes, %d total indexed assets)"),
                       *SourceLabel, *AssetInfoPath, *SourceKind, AddedEntryCount, AddedHashes.Num(),
            ...
            void HandleShaderLog(const FString &Message)
            {
                ...
                const int32 HashEnd =
                    Message.Find(TEXT("'"), ESearchCase::CaseSensitive, ESearchDir::FromStart, HashStart);
                ...
                UE_LOG(LogTajsGraphEarlyDiag, Warning,
                       TEXT("[ShaderDiag][ShaderResourceMiss] Platform=%s ShaderResourceHash=%s KeyKind=Unknown "
                            "DetailSource=LogFallback RecentMaterials=%s"),
                       Platform.IsEmpty() ? TEXT("<unknown>") : *Platform, *Hash, *JoinValues(MaterialSnapshot));
            }
            ...
            void HandleMaterialLog(const FString &Message, ELogVerbosity::Type Verbosity)
            {
                ...
                if (Message.Contains(TEXT("Caching uniform expressions for material: ")))
                {
                    const FString MaterialName =
                        Message.RightChop(FString(TEXT("Caching uniform expressions for material: ")).Len())
                            .TrimStartAndEnd();
                ...
                {
                    const FString MaterialName =
                        ExtractQuotedValue(Message, TEXT("Invalid shader map ID caching shaders for '"));
                    EmitMaterialFailure(TEXT("InvalidShaderMapId"), MaterialName,
                                        ResolveAssetPathForMaterial(MaterialName),
                                        ExtractQuotedValue(Message, TEXT("platform ")), Verbosity);
                    return;
                }
            ...
            void HandleAssetLog(const FString &Message)
            {
                ...
                const int32 PathEnd = Message.Find(TEXT(": Failed to compile Material"), ESearchCase::CaseSensitive,
                                                   ESearchDir::FromStart, PathStart);
            ...
            void HandleDefaultMaterialFailure(const FString &Message, ELogVerbosity::Type Verbosity)
            {
                ...
                const int32 CloseParen = Payload.Find(TEXT(")"), ESearchCase::CaseSensitive, ESearchDir::FromStart,
                                                      OpenParen == INDEX_NONE ? 0 : OpenParen + 1);
                ...
                if (OpenParen != INDEX_NONE && CloseParen != INDEX_NONE && CloseParen > OpenParen)
                {
                    ...
                    AssetPath = TajsGraphEarlyShared::NormalizeObjectPath(
                        Payload.Mid(OpenParen + 1, CloseParen - OpenParen - 1));
                }

                MaterialName = MaterialName.TrimStartAndEnd();
                EmitMaterialFailure(TEXT("DefaultMaterialMissing"), MaterialName, AssetPath, TEXT("PCD3D_SM6"),
                                    Verbosity);
            }

            void EmitMaterialFailure(const FString &EventName, const FString &MaterialName, FString AssetPath,
                                     const FString &PlatformFromMessage, ELogVerbosity::Type Verbosity)
            {
                ...
                const FString MatchState =
                    ExpectedHash.IsEmpty() ? TEXT("missing_expected_indexed_hash") : TEXT("missing_runtime_hash");
                ...
                const FString Key = FString::Printf(TEXT("MaterialFailure|%s|%s|%s|%s"), *EventName, *MaterialName,
                                                    *AssetPath, *Platform);
                if (!MarkEmitted(Key))
                {
                    return;
                ...
                if (Verbosity == ELogVerbosity::Error || Verbosity == ELogVerbosity::Fatal)
                {
                    UE_LOG(LogTajsGraphEarlyDiag, Error,
                           TEXT("[ShaderDiag][MaterialFailure] Event=%s Material=%s AssetPath=%s Platform=%s "
                                "RuntimeHash=<unavailable> RuntimeHashSource=LogFallback ExpectedIndexedHash=%s "
                                "Match=%s KeyKind=Unknown ShaderType=<unknown> Pipeline=<unknown> "
                                "VertexFactory=<unknown> RecentShaderResourceHashes=%s"),
                           *EventName, MaterialName.IsEmpty() ? TEXT("<unknown>") : *MaterialName,
                           AssetPath.IsEmpty() ? TEXT("<unknown>") : *AssetPath, *Platform,
                           ExpectedHash.IsEmpty() ? TEXT("<missing>") : *ExpectedHash, *MatchState,
            ...
            FString ResolveAssetPathForMaterial(const FString &MaterialName) const
            {
                ...
                {
                    if (TajsGraphEarlyShared::ExtractShortName(Pair.Key).Equals(MaterialName,
            ...
            static FString ExtractQuotedValue(const FString &Message, const FString &Prefix)
            {
                ...
                const int32 ValueEnd =
                    Message.Find(TEXT("'"), ESearchCase::CaseSensitive, ESearchDir::FromStart, ValueStart);
    ...
    bool Startup() { return GDiagnosticOutputDevice.Startup(); }

    void Shutdown() { GDiagnosticOutputDevice.Shutdown(); }

    bool IsEnabled() { return GDiagnosticOutputDevice.IsEnabled(); }
} // namespace ShaderMapDiagnostics
