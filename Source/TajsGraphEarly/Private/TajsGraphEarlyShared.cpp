...
namespace TajsGraphEarlyShared
{
    namespace
    {
        ...
        static FString ConvertPackagePathToObjectPath(const FString &PackagePath)
        {
            ...
            return ShortName.IsEmpty() ? FString() : FString::Printf(TEXT("%s.%s"), *PackagePath, *ShortName);
        }
    } // namespace

    const TCHAR *GetConfigSection() { return ConfigSection; }

    const TCHAR *GetEnableMaterialShaderMapDiagKey() { return EnableMaterialShaderMapDiagKey; }

    const TCHAR *GetEnableMaterialShaderMapHashRemapKey() { return EnableMaterialShaderMapHashRemapKey; }

    const TCHAR *GetEnableEngineDefaultMaterialOverridesKey() { return EnableEngineDefaultMaterialOverridesKey; }

    const TCHAR *GetDefaultMaterialOverrideKey() { return DefaultMaterialOverrideKey; }

    const TCHAR *GetDefaultDeferredDecalMaterialOverrideKey() { return DefaultDeferredDecalMaterialOverrideKey; }

    const TCHAR *GetDefaultLightFunctionMaterialOverrideKey() { return DefaultLightFunctionMaterialOverrideKey; }

    const TCHAR *GetDefaultPostProcessMaterialOverrideKey() { return DefaultPostProcessMaterialOverrideKey; }

    const TCHAR *GetWorldGridMaterialOverrideKey() { return WorldGridMaterialOverrideKey; }
    ...
    void GatherConfigCandidatePaths(TArray<FString> &OutConfigPaths)
    {
        ...
        {
            AddUniqueNonEmptyPath(OutConfigPaths,
                                  FPaths::Combine(Plugin->GetBaseDir(), TEXT("Config"), TEXT("Engine.ini")));
        }

        AddUniqueNonEmptyPath(OutConfigPaths, FPaths::Combine(FPaths::ProjectDir(), TEXT("Mods"), TEXT("TajsGraph"),
                                                              TEXT("Config"), TEXT("Engine.ini")));
    ...
    void GatherShaderDirCandidates(TArray<FString> &OutShaderDirs)
    {
        ...
        AddUniqueNonEmptyPath(OutShaderDirs, FPaths::ConvertRelativePathToFull(
                                                 FPaths::Combine(FPaths::ProjectDir(), TEXT("Mods"), TEXT("TajsGraph"),
                                                                 TEXT("Content"), TEXT("Shaders"))));

        if (TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(TEXT("TajsGraph")))
        {
            AddUniqueNonEmptyPath(OutShaderDirs, FPaths::ConvertRelativePathToFull(
                                                     FPaths::Combine(Plugin->GetContentDir(), TEXT("Shaders"))));
        }

        AddUniqueNonEmptyPath(OutShaderDirs,
                              FPaths::ConvertRelativePathToFull(FPaths::Combine(
                                  FPaths::ProjectPluginsDir(), TEXT("TajsGraph"), TEXT("Content"), TEXT("Shaders"))));
    }
    ...
    void GatherAssetInfoSearchDirs(const FString &PlatformName, TArray<FAssetInfoSearchDir> &OutSearchDirs)
    {
        ...
        OutSearchDirs.Add({FPaths::ConvertRelativePathToFull(FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("Shaders"),
                                                                             PlatformName + TEXT("-") + PlatformName)),
                           TEXT("dev_saved")});
        OutSearchDirs.Add(
            {FPaths::ConvertRelativePathToFull(FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("Cooked"),
                                                               TEXT("Windows"), TEXT("FactoryGame"), TEXT("Content"))),
             TEXT("dev_cooked_content")});
        OutSearchDirs.Add({FPaths::ConvertRelativePathToFull(
                               FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("Cooked"), TEXT("Windows"),
                                               TEXT("FactoryGame"), TEXT("Metadata"), TEXT("ShaderLibrarySource"))),
    ...
    FString NormalizeAssetPathFromFileSystem(const FString &FileSystemPath)
    {
        ...
        auto ConvertRelativeAssetPath = [](const FString &RelativePath, const TCHAR *RootPath)
        {
            ...
            return Directory.IsEmpty() ? FString::Printf(TEXT("%s%s"), RootPath, *BaseFilename)
