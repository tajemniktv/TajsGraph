...
namespace
{
    ...
    static const TArray<FEngineMaterialOverrideDefinition> &GetEngineMaterialOverrideDefinitions()
    {
        static const TArray<FEngineMaterialOverrideDefinition> Definitions = {
            {TajsGraphEarlyShared::GetWorldGridMaterialOverrideKey(), nullptr,
             TEXT("/Engine/EngineMaterials/WorldGridMaterial.WorldGridMaterial")},
            {TajsGraphEarlyShared::GetDefaultMaterialOverrideKey(), TEXT("DefaultMaterialName"),
             TEXT("/Engine/EngineMaterials/DefaultMaterial.DefaultMaterial")},
            {TajsGraphEarlyShared::GetDefaultDeferredDecalMaterialOverrideKey(),
             TEXT("DefaultDeferredDecalMaterialName"),
             TEXT("/Engine/EngineMaterials/DefaultDeferredDecalMaterial.DefaultDeferredDecalMaterial")},
            {TajsGraphEarlyShared::GetDefaultLightFunctionMaterialOverrideKey(),
             TEXT("DefaultLightFunctionMaterialName"),
             TEXT("/Engine/EngineMaterials/DefaultLightFunctionMaterial.DefaultLightFunctionMaterial")},
            {TajsGraphEarlyShared::GetDefaultPostProcessMaterialOverrideKey(), TEXT("DefaultPostProcessMaterialName"),
    ...
    static bool AreEngineMaterialOverridesEnabled()
    {
        ...
        return TajsGraphEarlyShared::ReadConfigBool(TajsGraphEarlyShared::GetEnableEngineDefaultMaterialOverridesKey(),
                                                    bEnableOverrides) &&
               bEnableOverrides;
    }
    ...
    static TArray<FResolvedEngineMaterialOverride> ResolveConfiguredEngineMaterialOverrides()
    {
        ...
        for (const FEngineMaterialOverrideDefinition &Definition : GetEngineMaterialOverrideDefinitions())
        {
            ...
            if (!TajsGraphEarlyShared::ReadConfigString(Definition.ConfigKey, RawOverridePath) ||
                RawOverridePath.TrimStartAndEnd().IsEmpty())
            {
                continue;
            }
            ...
            FString Error;
            if (!TajsGraphEarlyShared::NormalizeConfiguredObjectPath(RawOverridePath, ResolvedOverride.TargetObjectPath,
                                                                     ResolvedOverride.TargetPackagePath, Error))
            {
                UE_LOG(LogTajsGraphEarly, Warning, TEXT("[Startup][Overrides] Ignoring %s=%s (%s)"),
                       Definition.ConfigKey, *RawOverridePath, Error.IsEmpty() ? TEXT("invalid asset path") : *Error);
                continue;
            }
    ...
    static void LogConfiguredEngineMaterialOverrides(const bool bOverridesEnabled,
                                                     const TArray<FResolvedEngineMaterialOverride> &ResolvedOverrides)
    {
        ...
        if (ResolvedOverrides.IsEmpty())
        {
            UE_LOG(LogTajsGraphEarly, Warning,
                   TEXT("[Startup][Overrides] Enabled, but no valid default material overrides were configured."));
            return;
        }

        for (const FResolvedEngineMaterialOverride &ResolvedOverride : ResolvedOverrides)
        {
            UE_LOG(LogTajsGraphEarly, Display, TEXT("[Startup][Overrides] %s -> %s"),
    ...
    static void
    ApplyConfiguredDefaultMaterialOverrides(const bool bOverridesEnabled,
                                            const TArray<FResolvedEngineMaterialOverride> &ResolvedOverrides)
    {
        ...
        for (const FEngineMaterialOverrideDefinition &Definition : GetEngineMaterialOverrideDefinitions())
        {
            ...
            if (!bOverridesEnabled || !ActiveEngineSettingKeys.Contains(Definition.EngineSettingKey))
            {
                ...
                const bool bHasConfiguredValue =
                    GConfig->GetString(EngineConfigSection, Definition.EngineSettingKey, CurrentValue, GEngineIni);
                if (bHasConfiguredValue && CurrentValue.StartsWith(TajsGraphMaterialRoot, ESearchCase::CaseSensitive) &&
                    GConfig->RemoveKey(EngineConfigSection, Definition.EngineSettingKey, GEngineIni))
                {
                    UE_LOG(LogTajsGraphEarly, Display, TEXT("[Startup][Overrides] Cleared stale %s.%s from %s"),
                           EngineConfigSection, Definition.EngineSettingKey, *GEngineIni);
        ...
        for (const FResolvedEngineMaterialOverride &ResolvedOverride : ResolvedOverrides)
        {
            ...
            GConfig->SetString(EngineConfigSection, ResolvedOverride.Definition->EngineSettingKey,
                               *ResolvedOverride.TargetObjectPath, GEngineIni);
            UE_LOG(LogTajsGraphEarly, Display, TEXT("[Startup][Overrides] Set %s.%s=%s"), EngineConfigSection,
                   ResolvedOverride.Definition->EngineSettingKey, *ResolvedOverride.TargetObjectPath);
            bConfigModified = true;
        }
    ...
    static void ApplyConfiguredEngineMaterialRedirects(const TArray<FResolvedEngineMaterialOverride> &ResolvedOverrides)
    {
        ...
        for (const FResolvedEngineMaterialOverride &ResolvedOverride : ResolvedOverrides)
        {
            ...
            Redirects.Emplace(ECoreRedirectFlags::Type_Package,
                              FCoreRedirectObjectName(NAME_None, NAME_None,
                                                      *FPackageName::ObjectPathToPackageName(
                                                          FString(ResolvedOverride.Definition->SourceObjectPath))),
...
namespace ShaderHook
{
    ...
    static const TCHAR *BoolText(const bool bValue) { return bValue ? TEXT("true") : TEXT("false"); }
    ...
    static EShaderPlatform ResolveStartupShaderPlatform()
    {
        ...
        {
            UE_LOG(LogTajsGraphEarly, Warning,
                   TEXT("[Diag] GMaxRHIShaderPlatform is not initialized yet; falling back to SP_PCD3D_SM6 for early "
                        "shader loading."));
            return SP_PCD3D_SM6;
        }
    ...
    static TArray<FSupplementalShaderPack> DiscoverSupplementalShaderPacks(const FString &ShaderDir,
                                                                           const FString &PlatformName)
    {
        ...
        IFileManager::Get().FindFiles(
            ArchiveFiles, *FPaths::Combine(ShaderDir, TEXT("ShaderArchive-*.ushaderbytecode")), true, false);
        ArchiveFiles.Sort();
        ...
        for (const FString &ArchiveFile : ArchiveFiles)
        {
            ...
            const FString LibraryName =
                ArchiveFile.Mid(ArchivePrefix.Len(), ArchiveFile.Len() - ArchivePrefix.Len() - ArchiveSuffix.Len());
            ...
            Pack.CacheCandidates.Add(FPaths::Combine(
                ShaderDir, FString::Printf(TEXT("GlobalShaderCache-%s-%s.bin"), *LibraryName, *PlatformName)));
            ...
            {
                Pack.CacheCandidates.Add(
                    FPaths::Combine(ShaderDir, FString::Printf(TEXT("GlobalShaderCache-%s.bin"), *PlatformName)));
            }
    ...
    static void HookedCompileGlobalShaderMap(EShaderPlatform Platform, const ITargetPlatform *TargetPlatform,
                                             bool bRefreshShaderMap)
    {
        ...
        UE_LOG(LogTajsGraphEarly, Display,
               TEXT("[Hook] CompileGlobalShaderMap(%s) intercepted — InitForRuntime complete, loading tessellation "
                    "shaders"),
        ...
        if (FShaderCodeLibrary::IsEnabled())
        {
            ...
            if (PhysicalFile && (PhysicalFile != &FPlatformFileManager::Get().GetPlatformFile()))
            {
                ...
                UE_LOG(
                    LogTajsGraphEarly, Display,
                    TEXT("[Hook] Temporarily lowered platform file to PhysicalFile to access loose .ushaderbytecode."));
            }
            else if (!PhysicalFile)
            {
                UE_LOG(
                    LogTajsGraphEarly, Warning,
                    TEXT(
                        "[Hook] Could not find PhysicalFile platform — loose .ushaderbytecode may not be accessible."));
            }
            ...
            for (const FString &Dir : ShaderDirCandidates)
            {
                ...
                for (const FSupplementalShaderPack &Pack : Packs)
                {
                    ...
                    UE_LOG(LogTajsGraphEarly, Display, TEXT("[Hook] OpenLibrary(%s, %s) = %s"), *Pack.LibraryName, *Dir,
            ...
            {
                UE_LOG(LogTajsGraphEarly, Warning,
                       TEXT("[Hook] No supplemental shader archives were found in Content/Shaders — supplemental "
                            "global/project shader bytecode will be unavailable."));
                UE_LOG(LogTajsGraphEarly, Warning,
            ...
            {
                UE_LOG(LogTajsGraphEarly, Warning,
                       TEXT("[Hook] Supplemental shader archives were found but none opened successfully — "
                            "LoadResource may still fail for extra global permutations."));
            }
            ...
            // --- 2a: Load vanilla bin (Engine/ virtual root, same path as TryLoadCookedGlobalShaderMap) ---
            {
                ...
                if (FFileHelper::LoadFileToArray(Data, *VanillaBinPath, FILEREAD_Silent) && !Data.IsEmpty())
                {
                    ...
                    {
                        UE_LOG(LogTajsGraphEarly, Display,
                               TEXT("[Hook] Loaded vanilla GlobalShaderCache-%s (%.2f MB) from %s"), *PlatformName,
                               (float)Data.Num() / (1024.f * 1024.f), *VanillaBinPath);
                    }
                    else
                    {
                        UE_LOG(LogTajsGraphEarly, Warning,
                               TEXT("[Hook] Deserialisation error reading vanilla bin '%s' — FSR3/DLSS/XeSS "
                                    "permutations may be missing."),
                               *VanillaBinPath);
                    }
                }
                else
                {
                    UE_LOG(LogTajsGraphEarly, Warning,
                           TEXT("[Hook] Could not read vanilla bin from '%s' — FSR3/DLSS/XeSS permutations may be "
                                "missing."),
                           *VanillaBinPath);
                }
            }
            ...
            for (const FString &Dir : ShaderDirCandidates)
            {
                ...
                for (const FSupplementalShaderPack &Pack : Packs)
                {
                    ...
                    for (const FString &BinPath : Pack.CacheCandidates)
                    {
                        ...
                        if (!Reader.IsError())
                        {
                            UE_LOG(LogTajsGraphEarly, Display,
                                   ...
                                   *Pack.LibraryName, *PlatformName, (float)Data.Num() / (1024.f * 1024.f), *BinPath);
                    ...
                    if (!bLoadedPack && Pack.CacheCandidates.ContainsByPredicate(
                                            [](const FString &Candidate) { return FPaths::FileExists(Candidate); }))
                    {
                        UE_LOG(LogTajsGraphEarly, Warning,
                               TEXT("[Hook] Could not find a usable supplemental cache for '%s' in %s"),
                               *Pack.LibraryName, *Dir);
                    }
                }
            ...
            {
                UE_LOG(LogTajsGraphEarly, Display,
                       TEXT("[Hook] GGlobalShaderMap[%s] pre-populated (vanilla + %d supplemental pack(s)) — "
                            "CompileGlobalShaderMap will skip VerifyGlobalShaders."),
                       *PlatformName, LoadedSupplementalBinCount);
            }
            else
            {
                ...
                {
                    UE_LOG(LogTajsGraphEarly, Warning,
                           TEXT("[Hook] Could not load any supplemental global shader cache bins — the early "
                                "global-shader overlay will be unavailable."));
                }
                else
                {
                    UE_LOG(LogTajsGraphEarly, Warning,
                           TEXT("[Hook] No supplemental global shader cache bins were found — global-shader overlay is "
                                "inactive, but supplemental project/material shader archives may still be available."));
                }
        ...
        {
            UE_LOG(LogTajsGraphEarly, Warning,
                   TEXT("[Hook] FShaderCodeLibrary not enabled — tessellation shaders cannot be loaded. Startup will "
                        "likely crash."));
        }
    ...
    static bool InstallDetour()
    {
        ...
        {
            UE_LOG(LogTajsGraphEarly, Warning,
                   TEXT("[Hook] VirtualProtect failed — cannot patch CompileGlobalShaderMap"));
            HookTargetAddr = nullptr;
            OriginalCompileGSM = nullptr;
            return false;
        }

        UE_LOG(LogTajsGraphEarly, Display, TEXT("[Hook] CompileGlobalShaderMap detour installed at 0x%p"), Actual);
        return true;
    }
...
void FTajsGraphEarlyModule::LogD3D12Diagnostics(const TCHAR *PhaseLabel)
{
    ...
    {
        UE_LOG(LogTajsGraphEarly, Display,
               TEXT("[Diag][D3D12][%s] D3D12Core appears to be loaded from System32 (OS-selected runtime)."),
               PhaseLabel);
    }
    ...
    {
        UE_LOG(LogTajsGraphEarly, Display, TEXT("[Diag][D3D12][%s] EXE export D3D12SDKVersion=%u"), PhaseLabel,
               RequestedSdkVersion);
    }
    else
    {
    ...
    {
        UE_LOG(LogTajsGraphEarly, Display, TEXT("[Diag][D3D12][%s] EXE export D3D12SDKPath=%s"), PhaseLabel,
               *RequestedSdkPath);
    }
    else
    {
    ...
    {
        UE_LOG(LogTajsGraphEarly, Display, TEXT("[Diag][D3D12][%s] Loaded D3D12Core export D3D12SDKVersion=%u"),
               PhaseLabel, LoadedCoreSdkVersion);

        if (RequestedSdkVersion != 0 && RequestedSdkVersion != LoadedCoreSdkVersion)
        {
            UE_LOG(LogTajsGraphEarly, Warning,
                   TEXT("[Diag][D3D12][%s] Requested SDK version (%u) differs from loaded D3D12Core (%u)."), PhaseLabel,
                   RequestedSdkVersion, LoadedCoreSdkVersion);
        }
    }
    else
    {
        UE_LOG(
            LogTajsGraphEarly, Warning,
            TEXT("[Diag][D3D12][%s] Loaded D3D12Core export D3D12SDKVersion not found (or D3D12Core not loaded yet)."),
            PhaseLabel);
    }
#else
    UE_LOG(LogTajsGraphEarly, Display, TEXT("[Diag][D3D12][%s] Unsupported platform for Windows D3D12 diagnostics."),
           PhaseLabel);
#endif
}
...
void FTajsGraphEarlyModule::StartupModule()
{
    ...
    {
        UE_LOG(LogTajsGraphEarly, Display,
               TEXT("[Startup] Unreal Editor detected; skipping TajsGraphEarly runtime initialization."));
        return;
    }
#endif
    ...
    if (ShouldOverrideShaderVerification())
    {
        ...
        {
            UE_LOG(LogTajsGraphEarly, Display,
                   TEXT("[Startup] CompileGlobalShaderMap detour installed — shaders will load just before "
                        "VerifyGlobalShaders."));
        }
        else
        {
...
#else
    UE_LOG(LogTajsGraphEarly, Warning,
           TEXT("[Startup] Raw detour not implemented for this platform — shaders startup fix inactive."));
#endif
    ...
    UE_LOG(LogTajsGraphEarly, Display,
           TEXT("[Startup][Phase 5/5] Scheduling supplemental shader library registration."));
    PostEngineInitHandle =
        FCoreDelegates::OnPostEngineInit.AddRaw(this, &FTajsGraphEarlyModule::OpenShaderLibrariesPostInit);
...
void FTajsGraphEarlyModule::OpenShaderLibrariesPostInit()
{
    ...
    {
        UE_LOG(LogTajsGraphEarly, Warning,
               TEXT("[Library] FShaderCodeLibrary still not enabled at OnPostEngineInit — bytecode for tessellation "
                    "permutations cannot be registered. Rendering may fail."));
        return;
    }
    ...
    if (TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(TEXT("TajsGraph")))
    {
        ...
        UE_LOG(LogTajsGraphEarly, Display, TEXT("[Library] OpenLibrary(TajsGraph, %s) = %s"), *ContentDir,
               bTG ? TEXT("true") : TEXT("false"));
        if (!bTG)
        {
            UE_LOG(LogTajsGraphEarly, Display,
                   TEXT("[Library] TajsGraph material shader library already appears to be registered; treating "
                        "OpenLibrary=false as non-fatal."));
        }
        ...
        {
            const FString PlatformName =
                FDataDrivenShaderPlatformInfo::GetName(ShaderHook::ResolveStartupShaderPlatform()).ToString();
            const TArray<ShaderHook::FSupplementalShaderPack> Packs =
                ShaderHook::DiscoverSupplementalShaderPacks(ShadersDir, PlatformName);
            if (Packs.IsEmpty())
            {
                UE_LOG(LogTajsGraphEarly, Warning,
                       TEXT("[Library] No supplemental shader archives found in %s for platform %s"), *ShadersDir,
                       *PlatformName);
            }
            else
            {
                for (const ShaderHook::FSupplementalShaderPack &Pack : Packs)
                {
                    ...
                    UE_LOG(LogTajsGraphEarly, Display, TEXT("[Library] OpenLibrary(%s, %s) = %s"), *Pack.LibraryName,
                           *ShadersDir, bOpened ? TEXT("true") : TEXT("false"));
                    if (!bOpened)
                    {
                        if (FPaths::FileExists(Pack.ArchivePath))
                        {
                            UE_LOG(LogTajsGraphEarly, Display,
                                   TEXT("[Library] %s archive is present; OpenLibrary=false likely means it was "
                                        "already opened earlier or had no new components."),
                                   *Pack.LibraryName);
                        }
                        else
                        {
                            UE_LOG(LogTajsGraphEarly, Warning,
                                   TEXT("[Library] Could not open supplemental shader bytecode library '%s'."),
                                   *Pack.LibraryName);
                            UE_LOG(LogTajsGraphEarly, Warning, TEXT("[Library] Ensure %s is present in: %s"),
                                   *FPaths::GetCleanFilename(Pack.ArchivePath), *ShadersDir);
                        }
                    }
                }
            }
    ...
    {
        UE_LOG(LogTajsGraphEarly, Warning,
               TEXT("[Library] TajsGraph plugin not found via IPluginManager — cannot open shader libraries."));
    }
...
void FTajsGraphEarlyModule::LogShaderDiagnostics()
{
    ...
    if (FDataDrivenShaderPlatformInfo::IsValid(Platform))
    {
        ...
        UE_LOG(LogTajsGraphEarly, Display, TEXT("[Diag] Current Shader Platform (startup/provisional): %s"),
               *PlatformName);
    }
    else
    {
        UE_LOG(LogTajsGraphEarly, Display,
               TEXT("[Diag] Current Shader Platform: uninitialized (DDSPII not ready yet — expected in editor at "
                    "PostConfigInit)"));
    }
    ...
    {
        UE_LOG(LogTajsGraphEarly, Display,
               TEXT("[Diag] Feature Level: Uninitialized (expected during early startup)."));
    }

    // Log shader library status
    UE_LOG(LogTajsGraphEarly, Display, TEXT("[Diag] Shader library status at this phase: %s"),
           ShaderHook::BoolText(FShaderCodeLibrary::IsEnabled()));
    UE_LOG(LogTajsGraphEarly, Display,
           TEXT("[Diag] If required shaders are missing, this module attempts cache-based global map population."));
